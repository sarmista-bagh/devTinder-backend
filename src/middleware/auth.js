...
const userAuth = async (req, res, next) => {
  try {
    ...
    // Checks that the token was signed with the correct secret and is valid.

    // Find the user
    const { _id } = decodedObj;

    const user = await User.findById(_id);
    // MongoDB: find the user whose _id is the _id from the verified JWT.

    if (!user) {
      return res.status(404).send("User not found");
    }

    // Store user inside req
    req.user = user;
